...
import { Download, FileJson, Loader2, RefreshCw, Search, Sheet } from "lucide-react";
...
export function LogsTab() {
  ...
  const logs = useQuery({
    queryKey: ["admin-activity-log", category, days, search, page],
  ...
  const summary = useQuery({
    queryKey: ["admin-activity-log-summary", days],
    queryFn: async () => {
      const { data, error } = await supabase.rpc("admin_activity_log_summary", { _days: days });
      if (error) throw error;
  ...
  function exportRows(kind: "csv" | "json") {
    ...
    const name = `activity-log-${category}-${days}d`;
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-border">
        {logs.isLoading ? (
          ...
          <p className="p-4 text-sm text-muted-foreground">Nothing recorded for these filters yet.</p>
        ) : (
