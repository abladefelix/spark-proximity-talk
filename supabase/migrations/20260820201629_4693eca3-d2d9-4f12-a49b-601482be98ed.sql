...
ALTER TABLE public.locations
  DROP CONSTRAINT IF EXISTS locations_accuracy_m_check;
ALTER TABLE public.locations
  ADD CONSTRAINT locations_accuracy_m_check
...
RETURNS TABLE(
  ...
  updated_age_s double precision,
  is_pro boolean,
  beacon_style text
)
LANGUAGE plpgsql
...
BEGIN
  ...
  CROSS JOIN LATERAL (
    SELECT 6371000.0 * 2.0 * asin(least(1.0, sqrt(
      power(sin(radians(l.lat - mylat) / 2.0), 2) +
      cos(radians(mylat)) * cos(radians(l.lat)) *
      power(sin(radians(l.lng - mylng) / 2.0), 2)
    ))) AS dist
  ) d
  CROSS JOIN LATERAL (
    SELECT EXISTS (
